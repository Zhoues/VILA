...
from uniception.models.prediction_heads.base import AdaptorInput, PredictionHeadTokenInput
...
class SpatialQwen2Model(Qwen2Model):
    ...
    def build_metric_scale_factor_decoder(self, config):        
        spatial_tower = MapAnything.from_pretrained(config.spatial_tower) 
        self.scale_head = spatial_tower.scale_head.to(torch.device("cuda"))
        self.scale_adaptor = spatial_tower.scale_adaptor.to(torch.device("cuda"))
        for param in self.scale_head.parameters():
            param.requires_grad = False
        for param in self.scale_adaptor.parameters():
            param.requires_grad = False
        del spatial_tower
...
class SpatialQwen2ForCausalLM(Qwen2ForCausalLM):
    ...
    def get_metric_scale_factor_decoder(self):
        return getattr(self.model, "scale_head", None) and getattr(self.model, "scale_adaptor", None)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if labels is not None:
            ...
            # metric_scale_factor_loss = torch.tensor(0.0, dtype=cross_entropy_loss.dtype, device=cross_entropy_loss.device)
            ...
            #         # 5. 将 pred_metric_scale_factors_embeddings 转换为 tensor，形状为 (geo_num, out_dim), out_dim: 768
            #         pred_metric_scale_factors_embeddings = torch.stack(pred_metric_scale_factors_embeddings)

            #         # 6. 将 pred_metric_scale_factors_embeddings (geo_num, out_dim, 1)
            #         pred_metric_scale_factors_embeddings = pred_metric_scale_factors_embeddings.unsqueeze(-1)

            #         # 7. 通过 Decoder 获得对应的 metric scale factor, 形状为 (geo_num, 1)
            #         # NOTE(Zhouenshen): Now use scale_head and scale_adaptor to get the metric scale factor

            #         scale_head_output = self.model.scale_head(
            #             PredictionHeadTokenInput(last_feature=pred_metric_scale_factors_embeddings)
            #         )
            #         scale_final_output = self.model.scale_adaptor(
            #             AdaptorInput(
            #                 adaptor_feature=scale_head_output.decoded_channels,
            #                 output_shape_hw=(518, 518), # FIXME: hard code, as this value is not used
            #             )
            #         )
            #         pred_metric_scale_factors = scale_final_output.value.squeeze(-1)  # (B, 1, 1) -> (B, 1)
