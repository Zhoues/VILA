...
class DINOv2Encoder(nn.Module):
    ...
    def forward(self, image: torch.Tensor, token_rows: Union[int, torch.LongTensor], token_cols: Union[int, torch.LongTensor], return_class_token: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # NOTE(Zhouenshen): Half precision is not supported for antialias
        ...
        image_14 = F.interpolate(image, (token_rows * 14, token_cols * 14), mode="bilinear", align_corners=False, antialias=False)
        
        # NOTE(Zhouenshen): Same device for image_mean and image_std
        if image_14.device != self.image_mean.device or image_14.device != self.image_std.device:
            image_14 = image_14.to(self.image_mean.device)
